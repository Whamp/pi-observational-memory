...
Upstream independently hit the failure mode this gating prevents and patched only its emptiest branch: `elpapi42/pi-observational-memory` PR #36 ("fix: preserve context when memory is empty") and PR #39 ("fix/empty-compaction-fallback") decline compaction ownership **only when the rendered summary is empty** (`if (summary.length === 0) return`). That rule is safe for a fresh session with no memory yet, but it still returns a partial non-empty override when older memory omits newer, uncovered source — exactly the "non-empty older memory that omits newer source" case named in Considered options. The user-reported memory-loss bugs behind those PRs validate the decision to gate on **coverage reaching the prune boundary** rather than on summary emptiness: gating is strictly safer (covers the stale-memory case) and strictly cheaper (no model call on deliberately-covered empty spans). When contributing Compaction Authority upstream, cite #36/#39 and present this as the superseding design.
