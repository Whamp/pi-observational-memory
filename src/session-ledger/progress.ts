...
/** Returns the greatest valid Recorded or Empty Observation Coverage index. */
...
export function realTokensSinceAnchor(
	...
	// The observation clock must treat explicit Empty completions as coverage too
	// (ADR 0001); callers pass latestObservationCoverageIndex to include them.
	const coverageIdx = coverageIndexOverride ?? (customType ? latestCoverageIndex(entries, customType) : -1);
