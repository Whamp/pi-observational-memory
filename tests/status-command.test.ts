...
function setup(args: { entries: TestEntry[]; runtime?: Partial<any>; mode?: string; model?: unknown; contextUsage?: unknown }) {
...
describe("V3 /om:status", () => {
	...
	it("shows between-turn compaction with normal threshold progress", async () => {
		...
		expect(output).toContain("Next compaction:  ~2 / 30 estimated source tokens (7%)");
	});
